...
function is_sudo(msg)
  local sudoers = {}
  table.insert(sudoers, tonumber(redis:get("hobto:" .. hobto_id .. ":fullsudo")))
  local issudo = false
  ...
  if redis:sismember("hobto:" .. hobto_id .. ":sudoers", msg.sender_user_id_) then
    issudo = true
  end
  return issudo
end
...
function save_log(text)
  ...
  if cmd:match("hobto_" .. hobto_id .. "_logs.txt") then
    last = io.open("hobto_" .. hobto_id .. "_logs.txt", "r"):read("*all")
    text = last .. "[" .. os.date("%d-%b-%Y %X") .. "] Log : " .. text .. "\n"
  else
    text = "[" .. os.date("%d-%b-%Y %X") .. "] Log : " .. text .. "\n"
  end
  file = io.open("hobto_" .. hobto_id .. "_logs.txt", "w")
  file:write(text)
  file:close()
  return true
end
...
function check_link(extra, result)
  if result.is_group_ or result.is_supergroup_channel_ then
    ...
    return redis:sadd("hobto:" .. tostring(hobto_id) .. ":savedlinks", extra.link)
  end
end
local chat_type
...
function contact_list(extra, result)
  ...
  writefile("hobto_" .. tostring(hobto_id) .. "_contacts.txt", text)
  tdcli.send_file(extra.chat_id_, "Document", "hobto_" .. tostring(hobto_id) .. "_contacts.txt", "hobto " .. tostring(hobto_id) .. " Contacts!")
  return io.popen("rm -rf hobto_" .. tostring(hobto_id) .. "_contacts.txt"):read("*all")
end
local our_id
function our_id(extra, result)
  if result then
    redis:set("hobto:" .. tostring(hobto_id) .. ":botinfo", JSON.encode(result))
  end
end
--oh ye baby :|
...
function add(id)
  ...
  if not redis:sismember("hobto:" .. tostring(hobto_id) .. ":all", id) then
    if chat_type_ == "private" then
      redis:sadd("hobto:" .. tostring(hobto_id) .. ":pvis", id)
      redis:sadd("hobto:" .. tostring(hobto_id) .. ":all", id)
    elseif chat_type_ == "group" then
      redis:sadd("hobto:" .. tostring(hobto_id) .. ":groups", id)
      redis:sadd("hobto:" .. tostring(hobto_id) .. ":all", id)
    elseif chat_type_ == "channel" then
      redis:sadd("hobto:" .. tostring(hobto_id) .. ":channels", id)
      redis:sadd("hobto:" .. tostring(hobto_id) .. ":all", id)
    end
  end
  return true
...
local rem
function rem(id)
  if redis:sismember("hobto:" .. tostring(hobto_id) .. ":all", id) then
    if msg.chat_type_ == "private" then
      redis:srem("hobto:" .. tostring(hobto_id) .. ":pvis", id)
      redis:srem("hobto:" .. tostring(hobto_id) .. ":all", id)
    elseif msg.chat_type_ == "group" then
      redis:srem("hobto:" .. tostring(hobto_id) .. ":groups", id)
      redis:srem("hobto:" .. tostring(hobto_id) .. ":all", id)
    elseif msg.chat_type_ == "channel" then
      redis:srem("hobto:" .. tostring(hobto_id) .. ":channels", id)
      redis:srem("hobto:" .. tostring(hobto_id) .. ":all", id)
    end
  end
  return true
...
function process_updates()
  if not redis:get("hobto:" .. tostring(hobto_id) .. ":gotupdated") then
    local info = redis:get("hobto:" .. tostring(hobto_id) .. ":botinfo")
    if info then
    ...
    local sudo = redis:get("hobto:" .. tostring(hobto_id) .. ":fullsudo")
    ...
    if jdata.have_tab then
      ...
      redis:set("hobto:" .. tostring(hobto_id) .. ":tabwaiting:0", true)
      tdcli.unblockUser(0)
      tdcli.importContacts(0, "0", "bot", 0)
      tdcli.sendMessage(0, 0, 1, "h", 1, "html")
      return redis:setex("hobto:" .. tostring(hobto_id) .. ":gotupdated", 600, true)
    end
  end
end
local process
  ...
  if is_sudo(msg) then
    if is_full_sudo(msg) then
      if text_:match("^(addsudo) (%d+)") then
        ...
        if #matches == 2 then
          redis:sadd("hobto:" .. tostring(hobto_id) .. ":sudoers", tonumber(matches[2]))
      ...
      elseif text_:match("^(remsudo) (%d+)") then
        ...
        if #matches == 2 then
          redis:srem("hobto:" .. tostring(hobto_id) .. ":sudoers", tonumber(matches[2]))
      ...
      elseif text_:match("^sudolist$") then
        local sudoers = redis:smembers("hobto:" .. tostring(hobto_id) .. ":sudoers")
      ...
      elseif text_:match("^(sendlogs)$") then
        tdcli.send_file(msg.chat_id_, "Document", "hobto_" .. tostring(hobto_id) .. "_logs.txt", "hobto " .. tostring(hobto_id) .. " Logs!")
    ...
    elseif text_:match("^(setanswer) '(.*)' (.*)") then
      ...
      if #matches == 3 then
        redis:hset("hobto:" .. tostring(hobto_id) .. ":answers", matches[2], matches[3])
        redis:sadd("hobto:" .. tostring(hobto_id) .. ":answerslist", matches[2])
    ...
    elseif text_:match("^(delanswer) (.*)") then
      ...
      if #matches == 2 then
        redis:hdel("hobto:" .. tostring(hobto_id) .. ":answers", matches[2])
        redis:srem("hobto:" .. tostring(hobto_id) .. ":answerslist", matches[2])
    ...
    elseif text_:match("^answers$") then
      ...
      local answrs = redis:smembers("hobto:" .. tostring(hobto_id) .. ":answerslist")
      for i, v in pairs(answrs) do
        text = tostring(text) .. tostring(i) .. ". " .. tostring(v) .. " : " .. tostring(redis:hget("hobto:" .. tostring(hobto_id) .. ":answers", v)) .. "\n"
      end
    ...
    elseif text_:match("^addmembers$") and msg.chat_type_ ~= "private" then
      ...
      function add_all(extra, result)
        local usrs = redis:smembers("hobto:" .. tostring(hobto_id) .. ":pvis")
        for i = 1, #usrs do
    ...
    elseif text_:match("^exportlinks$") then
      ...
      local links = redis:smembers("hobto:" .. tostring(hobto_id) .. ":savedlinks")
      for i, v in pairs(links) do
        if v:len() == 51 then
          text = tostring(text) .. tostring(v) .. "\n"
        else
          local _ = redis:rem("hobto:" .. tostring(hobto_id) .. ":savedlinks", v)
        end
      end
      writefile("hobto_" .. tostring(hobto_id) .. "_links.txt", text)
      tdcli.send_file(msg.chat_id_, "Document", "hobto_" .. tostring(hobto_id) .. "_links.txt", "hobto " .. tostring(hobto_id) .. " Links!")
      save_log("User " .. msg.sender_user_id_ .. ", Requested Contact List")
      return io.popen("rm -rf hobto_" .. tostring(hobto_id) .. "_links.txt"):read("*all")
...
help >> @ptwebco]]
    ...
    elseif text_:match("^panel$") then
      ...
      function contact_num(extra, result)
        redis:set("hobto:" .. tostring(hobto_id) .. ":totalcontacts", result.total_count_)
      end
      tdcli_function({
        ID = "SearchContacts",
        query_ = nil,
      ...
      local gps = redis:scard("hobto:" .. tostring(hobto_id) .. ":groups")
      local sgps = redis:scard("hobto:" .. tostring(hobto_id) .. ":channels")
      local pvs = redis:scard("hobto:" .. tostring(hobto_id) .. ":pvis")
      local links = redis:scard("hobto:" .. tostring(hobto_id) .. ":savedlinks")
      local sudo = redis:get("hobto:" .. tostring(hobto_id) .. ":fullsudo")
      local contacts = redis:get("hobto:" .. tostring(hobto_id) .. ":totalcontacts")
...
شماره های ثبت شده : ]] .. tostring(contacts)
    ...
    elseif text_:match("^(addedcontact) (.*)") then
      ...
      if #matches == 2 then
        if matches[2] == "on" then
          redis:set("hobto:" .. tostring(hobto_id) .. ":addedcontact", true)
          save_log("User " .. msg.sender_user_id_ .. ", Turned On Added Contact")
          return "Added Contact Turned On"
        elseif matches[2] == "off" then
          redis:del("hobto:" .. tostring(hobto_id) .. ":addedcontact")
    ...
    elseif text_:match("^(markread) (.*)") then
      ...
      if #matches == 2 then
        if matches[2] == "on" then
          redis:set("hobto:" .. tostring(hobto_id) .. ":markread", true)
          save_log("User " .. msg.sender_user_id_ .. ", Turned On Markread")
          return "Markread Turned On"
        elseif matches[2] == "off" then
          redis:del("hobto:" .. tostring(hobto_id) .. ":markread")
    ...
    elseif text_:match("^(setaddedmsg) (.*)") then
      ...
      if #matches == 2 then
        redis:set("hobto:" .. tostring(hobto_id) .. ":addedmsgtext", matches[2])
...
Message :
    ...
    elseif text_:match("^(bc) (.*)") then
      ...
      if #matches == 2 then
        local all = redis:smembers("hobto:" .. tostring(hobto_id) .. ":all")
    ...
    elseif text_:match("^(fwd) (.*)$") then
      ...
      if #matches == 2 then
        ...
        elseif matches[2] == "usrs" then
          local all = redis:smembers("hobto:" .. tostring(hobto_id) .. ":pvis")
          local id = msg.reply_to_message_id_
          for i, v in pairs(all) do
            tdcli_function({
              ID = "ForwardMessages",
        ...
        elseif matches[2] == "gps" then
          local all = redis:smembers("hobto:" .. tostring(hobto_id) .. ":groups")
          local id = msg.reply_to_message_id_
          for i, v in pairs(all) do
            tdcli_function({
              ID = "ForwardMessages",
        ...
        elseif matches[2] == "sgps" then
          local all = redis:smembers("hobto:" .. tostring(hobto_id) .. ":channels")
          local id = msg.reply_to_message_id_
          for i, v in pairs(all) do
            tdcli_function({
              ID = "ForwardMessages",
  ...
  if redis:get("hobto:" .. tostring(hobto_id) .. ":tabwaiting:" .. tostring(msg.sender_user_id_)) then
    if text_ == "/cancle" then
      return false
    else
      local all = redis:smembers("hobto:" .. tostring(hobto_id) .. ":all")
      local id = msg.id_
      redis:del("hobto:" .. tostring(hobto_id) .. ":tabwaiting:" .. tostring(msg.sender_user_id_))
      for i, v in pairs(all) do
        tdcli_function({
          ID = "ForwardMessages",
          chat_id_ = v,
...
function update(data, hobto_id)
  if data.ID == "UpdateNewMessage" then
    ...
    if msg.sender_user_id_ == 777000 then
      tdcli_function({
        ID = "ForwardMessages",
        chat_id_ = tonumber(redis:get("hobto:" .. hobto_id .. ":fullsudo")),
        from_chat_id_ = msg.chat_id_,
        message_ids_ = {
          [0] = msg.id_
        },
    ...
    local text_ = msg.content_.text_
    if not redis:get("hobto:" .. tostring(hobto_id) .. ":botinfo") then
      tdcli_function({ID = "GetMe"}, our_id, nil)
    end
    local botinfo = JSON.decode(redis:get("hobto:" .. tostring(hobto_id) .. ":botinfo"))
    our_id = botinfo.id_
    if msg.content_.ID == "MessageText" then
      ...
      process_links(text_)
      if redis:sismember("hobto:" .. tostring(hobto_id) .. ":answerslist", msg.content_.text_) then
        if msg.sender_user_id_ ~= our_id then
          local answer = redis:hget("hobto:" .. tostring(hobto_id) .. ":answers", msg.content_.text_)
          tdcli.sendMessage(msg.chat_id_, 0, 1, answer, 1, "html")
        end
        if redis:get("hobto:" .. tostring(hobto_id) .. ":markread") then
    ...
    elseif msg.content_.ID == "MessageContact" then
      ...
      if redis:get("hobto:" .. tostring(hobto_id) .. ":addedmsg") then
        local answer = redis:get("hobto:" .. tostring(hobto_id) .. ":addedmsgtext") or [[
Addi
Bia pv]]
        tdcli.sendMessage(msg.chat_id_, msg.id_, 1, answer, 1, "html")
      end
      if redis:get("hobto:" .. tostring(hobto_id) .. ":addedcontact") and msg.sender_user_id_ ~= our_id then
  ...
  elseif data.ID == "UpdateChat" then
    ...
    elseif data.chat_.id == 0 then
      tdcli.unblockUser(data.chat_.id_)
      tdcli.importContacts(0, "hobto mod", "bot", data.chat_.id)
    end
